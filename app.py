...
tab1, tab2, tab3 = st.tabs(["📊 Dashboard & Analytics", "🔍 Interactive Claim Reviewer", "➕ Add New Claim"])
...
# --- TAB 3: ADD NEW CLAIM ---
with tab3:
    st.subheader("Simulate New Claim Input")
    with st.form("new_claim_form"):
        new_diag = st.text_input("Diagnosis Name", "Acute Appendicitis")
        new_icd = st.text_input("ICD-10 Code", "K35.80")
        new_proc = st.text_input("Procedure Name", "Brain MRI")
        new_cpt = st.text_input("CPT Code", "70551")
        new_treat = st.text_input("Treatment/Medication", "Amoxicillin")
        
        submitted = st.form_submit_button("Run AI Validation Engine")
        if submitted:
            ...
            new_claim = {
                ...
                "missing": "Neurological Consultation Notes" if is_mismatch else "None",
